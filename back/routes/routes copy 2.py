...
from models.models import Avocat, UserContext, db, Abonnee  # Assurez-vous d'importer le modèle Abonnee
...
def search_lawyer(specialization, ville):
    # Rechercher un avocat dans la table Avocat selon la spécialisation et la ville
    avocat = Avocat.query.filter_by(specialisation=specialization).filter(Avocat.adresse.ilike(f'%{ville}%')).first()
    return avocat
...
def traiter_question():
    ...
    # Gérer les tags spéciaux (greeting, introduce_self, goodbye, thank_you, chatbot_info, contact_avocat)
    # if predicted_tag == "contact_avocat":
    #     return jsonify({"message": "Pour des conseils juridiques personnalisés, veuillez contacter notre expert au 73535253."})
    # Si le tag est "contact_avocat", démarrer la gestion du dialogue
    
    if predicted_tag == "contact_avocat":
        # Générer un identifiant utilisateur temporaire si pas déjà dans la session
        if 'user_id' not in session:
            session['user_id'] = str(uuid.uuid4())  # Stocker l'ID temporaire dans la session

        user_id = session['user_id']  # Récupérer l'ID de l'utilisateur depuis la session
        user_context = UserContext.query.filter_by(user_id=user_id).first()

        try:
            if user_context is None:
                # Si aucun contexte, demander la spécialisation
                user_context = UserContext(user_id=user_id, step="waiting_specialisation")
                db.session.add(user_context)
                db.session.commit()
                return jsonify({"message": "Pouvez-vous préciser le domaine de droit en rapport avec votre problème ? (ex: droit du travail, droit pénal, droit civil...)"}), 200
                    
            # Si le chatbot attend la spécialisation
            elif user_context.step == "waiting_specialisation":
                user_context.specialisation = question  # Stocker la spécialisation fournie
                user_context.step = "waiting_ville"     # Passer à la demande de ville/quartier
                db.session.commit()
                return jsonify({"message": "Merci ! Dans quelle ville ou quel quartier êtes-vous situé(e) ?"}), 200

            # Si le chatbot attend la ville
            elif user_context.step == "waiting_ville":
                user_context.ville = question  # Stocker la ville fournie
                user_context.step = "search_lawyer"    # Passer à la recherche de l'avocat
                db.session.commit()

                # Rechercher l'avocat correspondant dans la base de données
                avocat = search_lawyer(user_context.specialisation, user_context.ville)
                if avocat:
                    response_message = f"L'avocat spécialisé en {user_context.specialisation} le plus proche de vous est {avocat.nom_prenom}. Vous pouvez le contacter au {avocat.telephone}."
                else:
                    response_message = "Désolé, nous n'avons pas trouvé d'avocat correspondant à votre recherche."

                return jsonify({"message": response_message}), 200

        except Exception as e:
            print(f"An error occurred: {e}")  # Affiche l'erreur dans la console
            return jsonify({"message": "Une erreur s'est produite lors du traitement de votre demande.", "error": str(e)}), 500
